...
PORT = 8000
SERVER = socket.gethostbyname(socket.gethostname()) #getting ip address here
ADDR = (SERVER, PORT)
		...
		if msg == "/viewRequests":
			...
			if username == groups[groupname].admin:
				...
				if usernameToApprove in groups[groupname].joinRequests:
					...
					print("\nMember Approved:",usernameToApprove,"|| Group:",groupname)
			...
			print("\nUser Disconnected:",username,"|| Group:",groupname)
			break
			...
			print("\nWaiting Client:",username,"Disconnected")
			break
			...
			if username == groups[groupname].admin:
				...
				if newAdminUsername in groups[groupname].allMembers:
					...
					print("\nNew Admin:",newAdminUsername,"| Group:",groupname)
			...
			if username == groups[groupname].admin:
				...
				if usernameToKick in groups[groupname].allMembers:
					...
					print("\nUser Removed:",usernameToKick,"| Group:",groupname)
			...
			print("\nFile received:",filename,"| User:",username,"| Group:",groupname)
			...
			print("\nFile sent",filename,"| Group: ",groupname)
			os.remove(filename)
...
def handle_client(client):
	...
	if groupname in groups:
		...
		if username in groups[groupname].allMembers:
			...
			client.send(b"/ready")
			print("\nUser Connected : ",username,"|| Room:",groupname)
			...
			client.send(b"/wait")
			print("\nJoin Request : ",username,"|| Room : ",groupname)
		...
		print("\nNew Room : ",groupname,"|| Admin: ",username)
...
def main():
	...
	while True:
		...
		client , _ = ServerSocket.accept() #waiting for client connection
