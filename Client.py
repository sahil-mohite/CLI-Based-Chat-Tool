...
def serverListen(serverSocket):
	...
	while True:
		...
		if msg == "/viewRequests":
			...
			state["alive"] = False
			state["inputMessage"] = False
			print("\n\t-----You have been kicked. Press any key to quit-----")
			break
...
def main():
	...
	if response == "/adminReady":
		print("\n----You have created the group",state["groupname"],"and You are ADMIN now----")
		state["alive"] = True

	elif response == "/ready":
		print("\n\n\tYou have joined the group",state["groupname"])
		state["alive"] = True

	elif response == "/wait":
		print("\n----Your request to join the group is pending. Admin approval is required----")
		print("\n---------Available Commands---------\n/1 -> Disconnect\n")
	...
	if state["alive"]:
		...
		print("\n---------Available Commands---------\n")
		print("/1 -> View Join Requests (Admin)")
		print("/2 -> Approve Join Requests (Admin)")
		print("/3 -> Disconnect from chat room")
		print("/4 -> View All Members")
		print("/5 -> View Online Group Members")
		print("/6 -> Transfer Adminship(Admin)")
		print("/7 -> Check Group Admin")
		print("/8 -> Kick Member (Admin)")
		print("/9 -> File Transfer")
